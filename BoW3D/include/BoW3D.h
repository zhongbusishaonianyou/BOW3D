...
namespace BoW3D
{
    ...
    class BoW3D: public unordered_map<pair<float, int>, unordered_set<pair<int, int>, pair_hash>, pair_hash>
     //Dimension value, Dimension ID; Frame ID, Descriptor ID
    {
        public:
            BoW3D(LinK3D_Extractor* pLinK3D_Extractor, ros::NodeHandle* nh);

            ~BoW3D();
            
            void update(Frame* pCurrentFrame);

            int loopCorrection(Frame* currentFrame, Frame* matchedFrame, vector<pair<int, int>> &vMatchedIndex, double & inlier_ratio,Eigen::Matrix3d &R, Eigen::Vector3d &t);

            void retrieve(Frame* pCurrentFrame, int &loopFrameId, Eigen::Matrix3d &loopRelR, Eigen::Vector3d &loopRelt,double & matched_count) ;           
        ...
        public :
            ...
            double revisit_thres;
            int num_nodes_exclude;
