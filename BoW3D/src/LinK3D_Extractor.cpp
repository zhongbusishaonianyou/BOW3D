...
namespace BoW3D
{
    LinK3D_Extractor::LinK3D_Extractor(ros::NodeHandle* nh) : nh(*nh) {
        
        nh->param("/bow3d/nScans", nScans, 64);
        nh->param("/bow3d/scanPeriod", scanPeriod, 0.1f);
        nh->param("/bow3d/minimumRange", minimumRange, 0.1f);
        nh->param("/bow3d/distanceTh", distanceTh, 0.4f);
        nh->param("/bow3d/matchTh", matchTh, 6);
        
        scanNumTh = std::ceil(nScans / 6.0f);
        ptNumTh = std::ceil(1.5f * scanNumTh);
    ...
    {    
        ...
        for(int i = 0; i < numArea; i++)
        {
            ...
            //remove clusters including little points
            int numCluster = curAreaCluster.size();
